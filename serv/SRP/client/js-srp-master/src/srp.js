...
var Mode;
(function (Mode) {
    Mode[Mode["RFC2945"] = 0] = "RFC2945";
    Mode[Mode["SRPTools"] = 1] = "SRPTools";
    Mode[Mode["GoSRP"] = 2] = "GoSRP";
})(Mode || (Mode = {}));
/** Known-safe prime fields. */
const knownPrimeFields = new Map([
 ...
 * @return {PrimeField}
 */
function findPrimeField(bits = 0) {
    if (bits === 0)
        bits = 2048;
...
class Srp {
    ...
    async verifier(I, p, salt) {
        ...
        const pf = this.pf;
        if (!salt)
            salt = randomBytes(pf.n);
        const x = this.m === Mode.GoSRP ?
            await this.hashInt(concatBytes(ih, ph, salt)) :
            await this.hashInt(concatBytes(salt, new Uint8Array(await hash(this.h, concatBytes(ih, new Uint8Array([0x3A]), ph)))));
    ...
    async newClient(I, p, a) {
        ...
        return new Client({
            ...
            k: await this.hashInt(concatBytes(bytesFromBigint(pf.N), pad(pf.g, Number(pf.n)))),
        });
    }
}
/**
...
class Client {
     ...
     */
    async generate(salt, B) {
        let Bn = bigintFromBytes(B);
        const pf = this.s.pf;
        if (Bn % pf.N === 0n) {
            throw new Error("Invalid server public key");
        }
        const u = await this.s.hashInt(concatBytes(pad(this.A, pf.n), pad(Bn, pf.n)));
        if (u === 0n) {
            throw new Error("Invalid server public key");
        }
        const x = this.s.m === Mode.GoSRP ?
            await this.s.hashInt(concatBytes(this.i, this.p, salt)) :
            await this.s.hashInt(concatBytes(salt, new Uint8Array(await hash(this.s.h, concatBytes(this.i, new Uint8Array([0x3A]), this.p)))));
        ...
        if (this.s.m === Mode.GoSRP) {
            ...
            this._M = new Uint8Array(await hash(this.s.h, concatBytes(this._K, bytesFromBigint(this.A), bytesFromBigint(Bn), this.i, salt, bytesFromBigint(pf.N), bytesFromBigint(pf.g))));
        }
        else {
            this._M = new Uint8Array(await hash(this.s.h, concatBytes(xorBytes(new Uint8Array(await hash(this.s.h, bytesFromBigint(pf.N))), new Uint8Array(await hash(this.s.h, bytesFromBigint(pf.g)))), new Uint8Array(await hash(this.s.h, this.i)), salt, bytesFromBigint(this.A), bytesFromBigint(Bn), this._K)));
        }
     ...
     */
    async serverOk(proof) {
        const enc = new TextEncoder();
        const h = enc.encode(toHex(new Uint8Array(await hash(this.s.h, concatBytes(this._K, this.M)))));

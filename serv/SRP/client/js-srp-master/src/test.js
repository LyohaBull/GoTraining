...
async function testClientSrpTools() {
    ...
    const srp = new Srp(Mode.GoSRP, Hash.SHA256, 1024);
    ...
    const client = await srp.newClient(encoder.encode("alice"), encoder.encode("password123"));
    alert(client.credentials())
    serv_res = prompt()
    arr = client.parseServerCredentials(serv_res)
    const M1 = await client.generate(arr[0], arr[1]);
    alert(M1)
    alert(client.K)
    alert(toHex(client.K))

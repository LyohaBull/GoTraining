...
// Encrypted base64
func EnPwdCode(pwdStr string, key []byte) string {
	pwd := []byte(pwdStr)
...
func main() {
	...
	k := h.Sum(nil)
	fmt.Printf("%x\n", k) /*
		temp, _ := hex.DecodeString("e0dd6d4b9ba0dfaf0821c9f186423638")
		res, _ := AesDeCrypt(temp, []byte(PwdKey))
		fmt.Println(res)*/
	//aes encryption
	fmt.Println(len(k))
	result, err := AesEcrypt([]byte("12345678"), k)
	if err != nil {
		log.Fatal(err)
	}
	fmt.Printf("%x\n", result)
	result2, _ := AesDeCrypt(result, k)
	fmt.Println(result2)
	//4f4d74c15e0ad4afb323a17927b1176ecb0c95ecbdf8e776ceb093499e3ff4c45157b007ae7dff1688ac2d2bf9fef28644922a1b3bbc6ef5881cb1ed0dff298a
	// {"User":"Алексей Смирнов","Data":"e0dd6d4b9ba0dfaf0821c9f186423638"}
	// aes decrypt
